...
class fperms(commands.Cog):
    ...
    async def options(self, ctx):
        permissions = ['ban_members', 'kick_members', 'administrator', 'manage_roles', 'manage_channels', 'moderate_members (perms to timeout)', 'manage_nicknames', 'manage_messages']
        flagged="\n".join(flag for flag in permissions)
        return await ctx.send(embed=discord.Embed(title=f"{self.urgentmoji} Fake Permissions", description=f"```ban_members\n{flagged}```", color=self.urgecolor))
