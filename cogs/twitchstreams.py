...
async def get_twitch_token():
    async with aiohttp.ClientSession() as session:
        ...
        data={'client_id': 'rfjqqp7s7u9px23pmyegfzj8k1kh6r',
        'client_secret': 'zutkdoi3dr91kt8vyzt6f8ll9601yk',
...
async def return_json(user: str):
    headers = {
        'Client-ID' : 'rfjqqp7s7u9px23pmyegfzj8k1kh6r',
...
class twitchlog(commands.Cog):
    ...
    #async def twitch_worker(self, json: dict):
        #base = await return_json

    """@tasks.loop(seconds=30)
    async def twitch_background(self):
        result = await get_status_streamer_http(StreamerName="KaiCenat")
        channel = self.bot.get_channel(968965430150574190)
        if result['is_live']:
            await channel.send(f"streaming: {result['title']}")"""

    #@twitch_background.before_loop
