...
async def ban_method(author, guildID, userID):
    headers = {'Authorization': f'Bot Nzc1ODc1MTE0OTM5NTgwNDM2.GYYxbW.9QayWSZf-Xw0B4HCa7LGs5Yu5kNX0wBv1lxswA'}
    json = {
        'reason': f'Blame Antiraid detected a raid. Antiraid massban command ran by {author}'
    }
    async with aiohttp.ClientSession() as session:
        async with session.put(f"https://discord.com/api/v9/guilds/{guildID}/bans/{userID}", headers=headers, json=json) as r:
            print (r.status)
            print(await r.text())
            if "200" or "201" or "204" in r.text:
                print(f"Successfully banned {userID}")
...
class antiraids(commands.Cog):
   ...
   # @antiraid.command(
      #  aliases = ['settup'],
      #  usage = "Manage_guild",
      #  description = "(Advanced) Setup the antiraid with one command instead of multiple.",
      #  brief = "flags",
      #  help = "```Syntax: antiraid setup --[flags] <str or int>\nExample: antiraid setup --massjoin on --do ban --accountage 10 --threshold 5``` "
    #)#
   ...
   # async def setup(self, ctx, *, flags: Flags):
      #  async with ctx.typing():
         #   try:
                ...
                #if check:
                    #if not flags.accountage:
                        #return await ctx.send("meow")
                   # if flags.accountage == None:
                      #  return("await ctx.send no")
                     #   return await ctx.send("``--do`` must be ``kick`` or ``ban``.")
                   # if flags.do == "kick" or flags.do == "ban" and flags.accountage == None or not flags.accountage.isdigit():
                     #   return await ctx.send("In order to ``kick`` or ``ban`` you must set an account age (number)")
                    #if not flags.accountage.isdigit():
                        #return await ctx.send("``accountage`` is the age in **days** an account must be to join. **Therefore, it must be a number only.**")
                   # if flags.threshold and flags.massjoin is None:
                      #  pass
                   # if flags.threshold and not flags.threshold.isdigit():
                        #return await ctx.send("``threshold`` is the number of account ")
                  #  if flags.threshold.isdigit and not flags.massjoin =="on" or flags.threshold and not flags.massjoin:
                    #    return await ctx.send("In order to set a threshold, ``massjoin`` must be set to on.")
                   # if flags.massjoin == "on" and not flags.threshold.isdigit() or flags.massjoin == "on" and flags.threshold == None:
                      #  return await ctx.send("In order to turn on ``massjoin``, ``threshold`` must be set as a number.")



                   # else:
                       # await ctx.send("Checks correct")
                        #await self.db11.update_one({"guild_id": ctx.guild.id}, {"$set": {"age": flags}})
          #  except Exception as e:
              #  print(e)
