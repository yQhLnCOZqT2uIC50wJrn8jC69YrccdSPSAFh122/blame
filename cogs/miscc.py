import discord, base64,re, difflib, arrow, aiofiles,unicodedata, os, aiohttp, humanize, datetime, pathlib, os, psutil, humanfriendly,sys, ast, inspect
...
devs = [714703136270581841, 352190010998390796, 236522835089031170, 386192601268748289, 753277825372389402]
errorcol = 0xA90F25
urgecolor = 0xF3DD6C
...
class miscc(commands.Cog):
    ...
    async def userinfo(self, ctx, *, member: Union[discord.Member, discord.User] = None):
        async with ctx.typing():
            ...
            if member in ctx.guild.members:
               ...
               #embed.add_field(name=f"__Dates__",
                                #value=
                                  #  f"\n**Registered:** {discord.utils.format_dt(member.created_at, style='f')} **({discord.utils.format_dt(member.created_at, style='R')})**"
                                  #  f"\n**Joined:** {discord.utils.format_dt(member.joined_at, style='f')} **({discord.utils.format_dt(member.joined_at, style='R')})**", inline=False)
                ...
                #if roles:
                  #  embed.add_field(name=f"**__Roles:__**",
        ...
        #try:
           # prem=0
           # if len(fetched_user.mutual_guilds) > 0:
              #  for fetched_user in fetched_user.mutual_guilds:
                   # try:
                        #if fetched_user.premium_since:
                           # prem=+1
                           # if prem > 0:
                               # if "<:BadgeNitro:925627225020194836>" not in user_flags or "<:BadgeNitro:925627225020194836>" not in badges:
                                 #   user_flags.append(f'<:BadgeNitro:925627225020194836>')
                    #except:
                       # pass
        #except:
    ...
    async def botinfo(self, ctx):
        async with ctx.typing():
            bc = int(await self.bot.redis.get('g1')) + int(await self.bot.redis.get('g2')) + int(await self.bot.redis.get("g3")) + int(await self.bot.redis.get("g4")) + int(await self.bot.redis.get("g5")) + int(await self.bot.redis.get("g6"))
            uc = int(await self.bot.redis.get('m1')) + int(await self.bot.redis.get('m2')) + int(await self.bot.redis.get('m3')) + int(await self.bot.redis.get("m4")) + int(await self.bot.redis.get("m5")) + int(await self.bot.redis.get("m6"))
            stats=await self.bot.db.command('dbStats')
            find = await self.db.find_one({"start": "June 8th, 2022"})
            count = find['count']
            rows=stats['dataSize']
            tables=stats['storageSize']
            id1="352190010998390796"
            id2="1082429362420326421"
            ...
            embed = discord.Embed(title=f"<:FFinfo:926308238322987058> blame",url="https://blame.gg", color=int(await get_theme(self, bot=self.bot, guild=ctx.guild.id), 16), description=f"**Developer** <@!{id2}>\n**Management** <@!386192601268748289>\n**Redefining versatility since:** {discord.utils.format_dt(self.bot.user.created_at, style='R')}")
            try:
                embed.add_field(name="__Statistics:__", value=f"Users: **{uc:,}**\nGuilds: **{bc:,}**")
                ...
                embed.set_footer(text=f"The Blame Team").set_thumbnail(url=self.bot.user.avatar.url)
    ...
    @commands.hybrid_command(aliases=['youtube', 'utube', 'ytsearch'])
    async def yt(self, ctx, *, search: str):
        async with ctx.typing():
            try:
                ...
                async with aiohttp.ClientSession() as s:
                    async with s.get('http://www.youtube.com/results?', params=params) as html_content:
                        ...
                        for i in search_results:
                            ...
                            msgs.append(f"<a:youtube:921863397623087184> **({len(msgs)}/{len(search_results)})** {msg} **({len(msgs)}/{len(search_results)})** <a:youtube:921863397623087184>")
                        paginator = pg.Paginator(self.bot, msgs, ctx, invoker=ctx.author.id)
                        if len(msgs) > 1:
                ...
                pass
                return await ctx.send('Something went wrong while trying to scrape')
