...
def is_owner(ctx):
    return ctx.message.author.id == 714703136270581841 or 236522835089031170 or 386192601268748289 or 753277825372389402



devs = [714703136270581841, 236522835089031170, 386192601268748289, 753277825372389402]
...
class blacklistSystem(commands.Cog):
    ...
    async def on_guild_join(self, guild):
        try:
            zzz = self.bot.get_channel(1016375342627307561)
            #for c in guild.text_channels:
                #if c.permissions_for(guild.me).create_instant_invite:
                    #invite = await c.create_invite()
            embed = discord.Embed(description=f'**added to {guild.name}** - ``{guild.id}`` with **{len(guild.members):,} member(s)** owned by **{guild.owner}**', colour=discord.Color.blurple())
...
#idk exactly how to use guidl ids
