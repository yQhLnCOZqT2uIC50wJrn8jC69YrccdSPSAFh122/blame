...
class mod(commands.Cog):
    ...
    @commands.command(name='massmove', aliases=['moveall'], description="move all members from current channel to another channel", brief='destination', usage='administrator', help="```Example: ;massmove 12418428```")
    @commands.has_permissions(administrator=True)
    async def massmove(self, ctx, destination: discord.VoiceChannel):
        """Syntax: !massmove <channel>
        Example: !massmove 8434834333"""
        ...
        if origin.members:
            if origin != destination:
                ...
                for member in origin.members:
                    ...
                    moved.append(member)
                await util.send_good(ctx, f"Moved `{len(moved)}` members from `{origin.name}` to `{destination.name}`")
            else:
                await util.send_error(ctx, f"You can't move people to the same voice channel that they're already in!")
        else:
            await util.send_error(ctx, f"{origin.name} is empty!")

    @commands.command(name='restore', brief='member', description="restore a member's roles after leaving", usage='manage_roles', help="```Example: ;restore @cop#0001```")
    @commands.guild_only()
    @commands.has_permissions(manage_roles=True)
    async def restore(self, ctx, member: discord.Member):
        ...
        try:
            counter=0
            rolenames=[]
            roles=self.restore[member.id][0]
            roles=roles.split(",")
            for rr in roles:
                roled = discord.utils.get(ctx.guild.roles, id=int(rr))
                rolenames.append(roled.name)
                try:
                    await member.add_roles(roled, reason=f"roles restored by {ctx.author}")
                    counter+=1
                except:
                    pass
            new_lst=(','.join(str(a)for a in rolenames))
            newroles=f"{new_lst}"
            return await util.send_good(ctx, f"added `{newroles}` to {member.mention}")
        except:
            return await ctx.reply(embed=discord.Embed(color=int(await get_theme(self, bot=self.bot, guild=ctx.guild.id), 16), description=f" {ctx.author.mention}: **No roles found to restore**"))
