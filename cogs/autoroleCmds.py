...
class autoRolE(commands.Cog):
    ...
    async def cacheAutoroles(self):
        ...
        print(f"{f.LIGHTRED_EX}[{f.RESET}{f.BLUE}Autoroles CACHED{f.RESET}{f.LIGHTRED_EX}]{f.RESET} {f.GREEN}->{f.RESET} {f.LIGHTGREEN_EX}{f.RESET}")
    ...
    @commands.command(name='testaaa')
    async def testaa(self,ctx):
    ...
    async def autorole_events(self, member):
        ...
        for i in results:
            ...
            if role is None:
                ...
                self.cacheAutoroles.restart()
            if role.position >= member.guild.me.top_role.position or not role.is_assignable():
                return
