   ...
   # async def on_voice_state_update(self, member, before, after):
     #   guildID = member.guild.id
       ...
       # voiceChannelID = data["voiceChannelID"]
        #if voiceChannelID is None:
         #   pass
       # else:
       #     voiceID = voiceChannelID
            #try:
              #  if after.channel.id == voiceID:
                #    data2 = await self.userdb.find_one({"userID": member.id})
                #    cooldown = data2["voiceChannel"]
                 #   if cooldown is None:
                  #      pass
                   # else:
                   #     await member.send("Creating channels too quickly you've been put on a 15 second cooldown!")
                   #     await asyncio.sleep(15)
                  #  data3 = await self.guilddb.find_one({"guild_id": guildID})
                   # data4 = await self.userdb.find_one({"userID": member.id})
                   # voiceCategoryID = data3["voiceCategoryID"]
                    #channelName = data4["channelName"]
                  #  channelLimit = data4['channelLimit']
                   # channelLimit2 =data3['channelLimit']

                   # if channelName and channelLimit is None:
                     #   name = f"{member.name}'s channel"
                       # if channelLimit2 is None:
                       #     limit = 0
                      #  else:
                   #         limit = channelLimit2
                   # else:
                      #  if channelLimit2 is None:
                       #     name = channelName
                      #      limit = channelLimit
                       # elif channelLimit2 is not None and limit == 0:
                         #   name = channelName
                       #     limit = channelLimit2
                      #  else:
                          #  name = channelName
                   ...
                   # user.usersetup(id)
                    #asyncio.sleep(1)
                  #  await self.userdb.update_one({ "userID": id }, { "$set": { "voiceChannel": channelID}})
                   # def check(a,b,c):
                   #     return len(channel2.members) == 0
                  #  await self.bot.wait_for('voice_state_update', check=check)
                   # await channel2.delete()
                    #await asyncio.sleep(3)
                    #await self.userdb.delete_one({ "userID": id})
            #except Exception as e:
                #print(e)
    @commands.Cog.listener()
    async def on_voice_state_update(self, member, before, after):
