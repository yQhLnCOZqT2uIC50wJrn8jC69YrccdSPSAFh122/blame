...
async def api_req(self, params, ignore_errors=False):
    ...
    params["api_key"] = "0658eb3fd6b559009f9dfa2695118965"
    params["format"] = "json"
    tries = 0 
    max_tries = 2
    while True:
...
async def get_commits(author, repository):
    ...
    async with aiohttp.ClientSession() as session:
        async with session.get(url, headers={'accept': 'application/vnd.github.v3.raw', 'authorization': 'token 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'
            }) as response:
